...
class Person:
    ...
    def __init__(self, name, mail=None, adress=None):
        self.name = name
        self.mail = mail
        self.adress = adress
...
def make_dict_of_persons(
    names, mails=None, adresses=None, exclusions=None, exclude_both_ways=True
):
    ...
    for idx in range(len(names)):
        ...
        # If mail are provided, include these
        if mails:
            mail = mails[idx]
        else:
            None
        if adresses:
            ...
            None
        print(f"{name} - {mail} : {adress}")
        persons[name] = Person(name, mail, adress)
        pprint(persons[name])
    # If exclusions provided, add these to the persons
    if exclusions:
        for excluded_pair in exclusions:
            print(persons[excluded_pair[0]])
...
class Santa:
    ...
    def __str__(self):
        if self.list:
            return "Santa has assigned gifts to the {:d} people on his list".format(
                len(self.persons)
            )
        else:
        ...
        if self.check_pair_valid(giver, reciever):
            ...
            if len(recievers) == 1:
                raise RecursionError(
                    "Invalid selection made, one invalid reciever left. Try again."
                )
            return self.select_pair(
                giver, recievers
            )  # Infinite recursion possible without above check
    ...
    def check_list_valid(self, gifts=None):
        ...
        for gift in gifts:
            if gift.giver == gift.reciever:
                print(
                    "Invalid combination: {:s} giving to themselves".format(
                        gift.giver.name
                    )
                )
                return False
            if gift.reciever in gift.giver.exclusions:
                print(
                    "Invalid combination: {:s} in exclusion list of {:s}".format(
                        gift.reciever.name, gift.giver.name
                    )
                )
                return False
        return True
    ...
    def make_a_list(self):
        ...
        while self.num_attempts <= self.max_num_attempts:
            ...
            try:
                ...
                continue
        raise ValueError(
            "Unable to find valid combination after {:d} attempts. Decrease number of exclusions or increase max_num_attempts.".format(
...
class Elves:
    ...
    def check_contactable(self, persons=None):
        ...
        for person in persons:
            if not persons[person].mail or not persons[person].adress:
                return False
        return True

    def generate_email_message(self, giver_name, reciever_name, receiver_address):
        return self.message.format(
            name=giver_name, gift_reciever=reciever_name, address=receiver_address
        )
    ...
    def send_email(self):
        ...
        list_of_email_recipients = [
            self.persons[person].name + " <" + self.persons[person].mail + ">"
        ...
        with smtplib.SMTP(self.smtp_server, port) as server:
            ...
            server.starttls()
            # to know hwo to use the app with python : https://stackoverflow.com/questions/60975490/how-does-one-send-an-e-mail-from-python-not-using-gmail
            server.login("mail@address.com", "app_password")
            # server.login(email_address, password)
            for gift in self.gifts:
                giver_email = gift.giver.mail
                email_message = self.generate_email_message(
                    gift.giver.name, gift.reciever.name, gift.reciever.adress
                )
...
def main():
    ...
    for name in data["NAMES"]:
        if "," in name:
            ...
            mails.append(split_name[1].strip())
            addresses.append(split_name[2].strip())
    exclusions = []
    ...
    # Make the list
    persons = make_dict_of_persons(
        names, mails, addresses, exclusions, exclude_both_ways=exclude_both_ways
    )
    santa = Santa(persons)
